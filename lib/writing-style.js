...
function analyzeText(text) {
  ...
  const avgParagraphLength = paragraphs.length ? sentences.length / paragraphs.length : 0;
  const avgWordLength = words.length ? words.reduce((sum, w) => sum + w.length, 0) / words.length : 0;

  const formalMarkers = /\b(therefore|however|furthermore|moreover|thus|hence|accordingly)\b/gi;
...
function getWritingStyleProfile(textChunks) {
  ...
  const excerpts = valid.slice(0, 3).map((t) => t.slice(0, 300).trim() + (t.length > 300 ? '...' : ''));
  const summary = [
    `Tone: ${stats.tone}`,
