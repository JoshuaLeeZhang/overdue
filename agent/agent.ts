		...
		try {
			...
			await page.addInitScript(() => {
				const inject = () => {
					...
					btn.style.cssText =
						"position:fixed;bottom:16px;right:16px;z-index:2147483647;padding:10px 16px;background:#2563eb;color:white;border:none;border-radius:8px;font-size:14px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.2);";
					btn.onclick = () => {
						(
							window as unknown as { __agentCapture?: () => void }
						).__agentCapture?.();
						btn.remove();
					};
			...
			log(
				'Log in if needed, navigate to the page, then click "Capture page" when ready.',
			);
