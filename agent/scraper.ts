...
function isSameOriginAndRelevant(url: string, baseHost: string): boolean {
  try {
    ...
    if (u.pathname.includes('/logout') || u.pathname.includes('/sign_out')) return false;
    return true;
  } catch {
    return false;
  }
...
async function extractLinks(page: import('playwright').Page, baseUrl: string): Promise<string[]> {
  ...
  const links = await page.evaluate((host) => {
    ...
    for (const a of anchors) {
      ...
      try {
        ...
        if ((u.protocol !== 'https:' && u.protocol !== 'http:') || !u.hostname.includes(host)) continue;
        if (u.pathname.includes('/logout') || u.pathname.includes('/sign_out')) continue;
        seen.add(href);
        out.push(href);
      } catch {
        /* skip */
  ...
  return links.filter((href) => isSameOriginAndRelevant(href, baseHost)).slice(0, MAX_TRAVERSE_LINKS);
}
...
async function run(): Promise<void> {
  ...
  console.log('Launching browser', useProfile ? '(persistent profile for LEARN)' : '(ephemeral)', traverse ? '(traverse mode)' : '');
  ...
  try {
    ...
    if (traverse && urls.length > 0) {
      ...
      try {
        ...
        await page.goto(first, { waitUntil: 'domcontentloaded', timeout: 20000 });
        ...
        const toVisit = links.filter((u) => !seenUrls.has(u)).slice(0, MAX_TRAVERSE_LINKS - 1);
        ...
        for (const link of toVisit) {
          try {
            ...
            pages.push({ url: link, title: '', text: `Error: ${(err as Error).message}` });
          }
        }
      } catch (err) {
        pages.push({ url: first, title: '', text: `Error: ${(err as Error).message}` });
      }
    } else {
