...
export async function runDedalusAgent(
	options: DedalusRunOptions,
): Promise<DedalusRunResult> {
	...
	if (!process.env.DEDALUS_API_KEY) {
		throw new Error(
			"DEDALUS_API_KEY is required. Get it from https://www.dedaluslabs.ai/dashboard/api-keys",
		);
	}
	...
	if (
		typeof (result as AsyncIterableIterator<unknown>)[Symbol.asyncIterator] ===
		"function"
	) {
		throw new Error(
			"Streaming response not supported in this endpoint; use stream: false",
		);
	}
