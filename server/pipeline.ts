...
export async function generateDraft(
	...
	const instructions =
		assignmentSpec.instructions || "No instructions provided.";
	const pages = (context && context.pages) || [];
	const styleSummary =
		(styleProfile && styleProfile.summary) || "No writing style provided.";
	const excerpt =
		styleProfile && styleProfile.excerpts && styleProfile.excerpts[0]
...
export function prepareFormFill(
	...
	const url =
		assignmentSpec.url ||
		(context && context.pages && context.pages[0] && context.pages[0].url) ||
		"";
	return {
		url,
		values: {},
