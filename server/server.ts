...
app.post("/run-agent", async (req, res) => {
	...
	if (hasDedalus && (input || !job)) {
		...
		try {
			broadcast({
				type: "log",
				payload: "Running Dedalus agent (LLM + Browser Use MCP)...",
			});
...
function runScraper(
	urls: string[],
	traverse = false,
): Promise<{ pages: { url: string; title: string; text: string }[] }> {
		...
		function processLine(line: string): void {
			...
			try {
				...
				if (parsed.result != null && parsed.result.context != null) {
					result = parsed.result.context as {
						pages: { url: string; title: string; text: string }[];
					};
				}
			} catch {
...
app.post("/scraper/login", (_req, res) => {
	...
	res.status(202).json({
		ok: true,
		message:
			"Login browser opened for learn.uwaterloo.ca. Log in, then close the window when done.",
	});
});
...
app.post("/pipeline/draft", async (req, res) => {
	...
	const context = contextBody ||
		(contextId ? await store.get(contextId) : null) || { pages: [] };
	try {
