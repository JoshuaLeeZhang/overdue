...
async function createConvexStore(): Promise<ContextStore | null> {
  ...
  try {
    ...
    return {
      async get(id: string) {
        const doc = await (client as { query: (name: string, args: object) => Promise<{ pages?: ContextPage[] } | null> }).query(
          'contextStore:get',
          { id }
        );
      ...
      async set(ctx: StoredContext) {
        const id = await (client as { mutation: (name: string, args: object) => Promise<string> }).mutation(
          'contextStore:store',
          { pages: ctx.pages ?? [] }
        );
        return id;
    ...
    if (process.env.CONVEX_URL || process.env.CONVEX_DEPLOYMENT) {
      console.warn('Convex context store unavailable, using in-memory:', (err as Error).message);
    }
    return null;
  }
}
