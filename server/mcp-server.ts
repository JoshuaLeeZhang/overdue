...
async function runAgentAndGetResult(
	...
	if (!runRes.ok) {
		throw new Error(
			`Backend returned ${runRes.status}. Is the server running? Start it with: npm run server`,
		);
	}
...
async function main(): Promise<void> {
	...
	server.registerTool(
		...
		{
			description:
				"Run the Playwright agent to extract the page title and main text from a URL. Opens a visible Chromium window.",
			inputSchema: {
				url: z
					.string()
					.url()
					.optional()
					.describe("URL to open (default: https://learn.uwaterloo.ca)"),
			},
		},
	...
	server.registerTool(
		...
		{
			description:
				"List files in Google Drive (optionally by folder and mime type). Requires Google OAuth configured.",
			inputSchema: {
				...
				mimeType: z
					...
					.optional()
					.describe(
						"Filter by mime type, e.g. application/vnd.google-apps.document for Docs",
					),
			},
		},
		async (opts?: {
	...
	server.registerTool(
		...
		{
			description:
				"Analyze writing style from one or more Google Doc IDs; returns a style profile (tone, sentence length, etc.).",
			inputSchema: {
				docIds: z
					.array(z.string())
					.describe("Array of Google Doc file IDs to analyze"),
			},
		},
		async ({ docIds }: { docIds: string[] }) => {
			const out = await gdrive.getWritingStyle(docIds);
			if (out.error) return { content: [{ type: "text", text: out.error }] };
			const p: WritingStyleProfile = out.profile ?? {
				summary: "",
				stats: null,
			};
			const text = [
	...
	server.registerTool(
		...
		{
			description:
				"List files in Office 365 / OneDrive. Requires Microsoft OAuth configured.",
			inputSchema: {
				folderId: z.string().optional().describe("Folder ID (default: root)"),
		...
		async (opts?: { folderId?: string; pageSize?: number }) => {
			...
			if (out.error) return { content: [{ type: "text", text: out.error }] };
			const text =
				(out.files || []).map((f) => `${f.name} (${f.id})`).join("\n") ||
				"No files.";
			return { content: [{ type: "text", text }] };
		},
	);
	...
	server.registerTool(
		...
		{
			description:
				"Get plain text content of a Word document by OneDrive item ID.",
			inputSchema: {
	...
	server.registerTool(
		...
		{
			description:
				"Analyze writing style from one or more Office 365 document item IDs.",
			inputSchema: {
				itemIds: z
					.array(z.string())
					.describe("Array of OneDrive item IDs to analyze"),
			},
		},
	...
	server.registerTool(
		...
		{
			description:
				"Scrape one or more URLs (assignment page, rubric, linked readings) and return structured context. Set traverse=true to follow links from the first URL and scrape assignment subpages.",
			inputSchema: {
				...
				traverse: z
					...
					.optional()
					.describe(
						"If true, follow links from the first URL to scrape assignment pages (default: false)",
					),
			},
		},
		async ({ urls, traverse }: { urls: string[]; traverse?: boolean }) => {
			...
			const summary = context.pages
				? context.pages
						.map(
							(p) =>
								`${p.url}: ${p.title || "no title"} (${(p.text || "").length} chars)`,
						)
						.join("\n")
	...
	server.registerTool(
		...
		{
			description:
				"Navigate to a URL and fill form fields. Pass url and values (object: field name -> value). Uses Playwright; browser window is visible.",
			inputSchema: {
				url: z.string().url().describe("Page URL with the form"),
				values: z
					.record(z.string(), z.union([z.string(), z.number()]))
					.describe("Field names (name or id) to values to fill"),
			},
		},
		async ({
			url,
			...
			const runRes = await fetch(`${BACKEND_URL}/run-agent`, {
				...
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					job: { mode: "fill_form", url, values: values || {} },
				}),
			});
	...
	server.registerTool(
		...
		{
			description:
				"Generate an assignment draft from instructions, scraped context (by contextId or inline), and optional writing-style profile (or text chunks to derive style).",
			inputSchema: {
				instructions: z.string().optional().describe("Assignment instructions"),
				contextId: z
					.string()
					.optional()
					.describe("Context ID from scrape_assignment_context"),
				urls: z
					.array(z.string().url())
					.optional()
					.describe("If no contextId, scrape these URLs first"),
				styleProfile: z
					.object({
				...
				textChunks: z
					.array(z.string())
					.optional()
					.describe("Raw text to compute writing style from"),
			},
		},
		async ({
			instructions,
			...
			let context: { pages?: { url: string; title: string; text: string }[] } =
				{ pages: [] };
			if (contextId) {
		...
		"do_assignment",
		{
			...
			inputSchema: {
				assignmentUrl: z
					.string()
					.url()
					.describe("Main assignment page URL to scrape"),
				extraUrls: z
					.array(z.string().url())
					.optional()
					.describe("Additional URLs (rubric, readings)"),
				instructions: z
					.string()
					.optional()
					.describe("Override or add assignment instructions"),
				gdriveDocIds: z
					.array(z.string())
					.optional()
					.describe("Google Doc IDs to derive writing style from"),
			},
		},
		async ({
			assignmentUrl,
			...
			const { context } = (await scrapeRes.json()) as {
			...
			const draftRes = await fetch(`${BACKEND_URL}/pipeline/draft`, {
				...
				body: JSON.stringify({
					assignmentSpec: {
						instructions:
							instructions ||
							"Complete the assignment using the scraped context.",
					},
					context,
					styleProfile,
				}),
